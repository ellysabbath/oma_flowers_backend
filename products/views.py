...
class CategoryListView(generics.ListCreateAPIView):
    ...
    Open access (AllowAny), no pagination.
    """
...
class ProductListView(generics.ListCreateAPIView):
    ...
    Open access (AllowAny), no pagination.
    Each product may have a `seller` (Distributor) who earns PBV/CGV on sale.
    """
    queryset = Product.objects.select_related(
        'category', 'seller__user'          # 👈 NEW — prefetch seller + its user
    ).all()
    permission_classes = [permissions.AllowAny]
    filter_backends = [filters.SearchFilter, filters.OrderingFilter]
    search_fields = ['name', 'sku', 'category__name', 'seller__user__email']
